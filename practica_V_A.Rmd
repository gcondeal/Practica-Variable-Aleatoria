...
```{r}
integ <- integrate(function(x) (25-x), 5, 25)$value
integ
```
...
probabilidad <- integrate(f,20,25)$value
probabilidad
```

*La probabilidad de que un cliente obtenga más de un 20% de descuento es de *`r probabilidad` *, es decir, un *`r probabilidad*100`*%.*
...
```{r}
esperanza <-integrate(function(x) x*f(x),5,25)$value
esperanza
```

*El descuento medio que se espera será de *`r esperanza`*.*
...
```{r}
Ex2<- integrate(function(x) x^2*f(x),5,25)$value
Ex_2 <- esperanza^2
varianza <- Ex2 - Ex_2
varianza
```

*Y obtenemos el valor de la varianza: * `r varianza`*.*
...
*El intervalo se comprende entre* `r lim_menor` *y* `r lim_mayor`.
...
*Obtendremos dicho valor de forma gráfica:*
...
y_minimo <- 0 
y_maximo <- 0.15
x_limites <-c(x_minimo,x_maximo)
y_limites <-c(y_minimo,y_maximo)
...
#Introducimos los ceros en los intervalos fuera del rango definido en los datos(5 -25)
seq_valores_func[((0/intervalos)+1):((5/intervalos))] <- 0
seq_valores_func[((25/intervalos)+1):((30/intervalos)+1)] <- 0

#Creamos DF
valores <- data.frame(valores_x = seq_valores, f_x= seq_valores_func)

#el grafico de la función de densidad
f_densidad <- ggplot(valores, aes(valores_x)) +
  geom_line(aes(y = f_x), colour = "blue", size = 1) +
  lims(x = x_limites, y = y_limites) + 
  ggtitle("Función de densidad") + 
  ylab("f(x)") +  xlab(bquote(~x[i]))

#para la representación del máximo
valor_max <- max(valores$f_x)
valores <- data.frame(valores, max_func = valor_max)

f_densidad + 
  geom_line(aes(y = valores$max_func))

```

*El valor que nos da el máximo y por tanto valor de la moda, sería el 5.*

*Para validar el dato, comprobaremos el resultado en los datos calculados*
